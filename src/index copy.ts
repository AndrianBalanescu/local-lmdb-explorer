...
function listCollections(dbPath: string): string[] {
  try {
    const rootDb = open({ path: dbPath, maxDbs: 1024 });
      ...
      async POST(req) {
        try {
          ...
          try {
            ...
            const rootDb = open({ path: absolutePath, maxDbs: 1024 });
            const keys = Array.from(rootDb.getKeys());
    ...
    "/api/databases/:dbName/search": async (req) => {
      try {
        ...
        try {
          ...
          // Reuse cached database handle or open new one
          const rootDb = dbConfig?.dbHandle || open({ path: dbPath, maxDbs: 1024 });
          const keys = Array.from(rootDb.getKeys());
          ...
          // Process each sub-database using xlmdb
          for (const key of keys) {
            try {
              // Use xlmdb to open sub-database
              const subDb = db<any>(dbPath, String(key));
              ...
              console.log(`Failed to read sub-database ${String(key)}:`, subDbError);
            }
          }
    ...
    "/api/seed": {
      async POST() {
        try {
          ...
          const rootDb = open({ path: DB_PATH, maxDbs: 1024 });
